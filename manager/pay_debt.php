

<div id="global">
            
<div class="container-fluid">



<?php

	if(isset($_GET["s_id"])){   $get_supplier_id = $_GET["s_id"] ?>

	<div class="row cm-fix-height">
		<div class="col-lg-12 col-md-12">
                        <div class="panel panel-default">
                            <div class="panel-heading">List Of Suppliers</div>
                            <div class="panel-body">
                               <table class="table" id="my_selected_table">
                                <thead>
                                    <tr>
                                        <th>Name</th>
                                        <th>Product</th>
                                        <th>Unit Cost</th>
                                        <th>Quantity Purchased</th>
                                        <th>Total Cost</th>
                                        <th>Amount Paid</th>
                                        <th>Amount Due</th>
                                        <th>Date Ordered</th>
                                  
                                    </tr>
                                </thead>
                                <tbody id="add_category_table">
                                   <?php  
                                    $get_orders_query = mysqli_query($connection,"SELECT * from order_table join stock_product_table on order_table.order_product_name = stock_product_table.stock_product_id join tblsuppliers on order_table.order_supplier_name = tblsuppliers.supplierid where order_table.order_supplier_name = '$get_supplier_id'");

                                     $my_counter = 0;
                                    while ($get_each_debt = mysqli_fetch_array($get_orders_query)) {?>
                                        <tr>
                                            <td><?php echo $get_each_debt["orgname"]; ?></td>  
                                            <td><?php echo $get_each_debt["stock_product_name"]; ?></td>
                                            <td><?php echo $get_each_debt["order_unit_cost"]; ?></td>
                                            <td><?php echo $get_each_debt["order_quantity_buying"]; ?></td>
                                            <td><?php echo $get_each_debt["order_total_cost"]; ?></td>
                                            <td><?php echo $get_each_debt["order_amount_payed"]; ?></td>
                                            <td><?php echo $get_each_debt["order_amount_due"]; ?></td>
                                            <td><?php echo $get_each_debt["order_timestamp"]; ?></td>
                                            
                                            <?php
                                            $my_counter += $get_each_debt['order_amount_due'];
                                                ?>
                                        </tr>
                                  <?php  }  ?>
                                   
                                </tbody>
                                <tfoot>
                                	<tr style="background-color: #f6f6f6">
                                		<td colspan="6" style="font-weight: bolder;text-align: right"><strong><h4>Total Cost</h4></strong></td>
                                		<td><strong><h4>GH &nbsp;<?php echo $my_counter;  ?></h4></strong></td>
                                		<td>&nbsp;</td>
                                	</tr>
                                </tfoot>
                            </table>
                            </div>
                        </div>
                    </div>
                </div>


<?php	}else{ ?>
		<div class="row cm-fix-height">
		<div class="col-lg-12 col-md-12">
                        <div class="panel panel-default">
                            <div class="panel-heading">List Of Suppliers</div>
                            <div class="panel-body">
                               <table class="table" id="my_selected_table">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th>Name</th>
                                        <th>Contact</th>
                                        <th>Number</th>
                                        <th>Amount Due</th>
                                  
                                    </tr>
                                </thead>
                                <tbody id="add_category_table">
                                   <?php  
                                    $get_supplier_query = mysqli_query($connection,"SELECT * FROM tblsuppliers");
                                    while ($get_each_supplier = mysqli_fetch_array($get_supplier_query)) {    $supplier_id = $get_each_supplier['supplierid'];        ?>
                                        <tr>
                                            <td><?php echo $supplier_id; ?></td>  
                                            <td><?php echo $get_each_supplier["orgname"]; ?></td>
                                            <td><?php echo $get_each_supplier["contact_person"]; ?></td>
                                            <td><?php echo $get_each_supplier["tel"]; ?></td>
                                            
                                             <td><?php  $get_amount_due = mysqli_query($connection,"SELECT * FROM `order_table` WHERE `order_supplier_name` = '$supplier_id' AND `order_amount_due` > 0");

                                             $my_counter = 0;

                                             while ($get_number = mysqli_fetch_array($get_amount_due)) {
                                             	$my_counter += $get_number["order_amount_due"];
                                             }

                                             echo "<a href='index.php?pay_debt&s_id=$supplier_id' target='_blank' style='text-decoration:none'><strong>&nbsp;GH&nbsp;".$my_counter."</sctrong></a>";


                                                ?></td>
                                        </tr>
                                  <?php  }  ?>
                                   
                                </tbody>
                            </table>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="row cm-fix-height">
                    <div class="col-lg-7 col-lg-offset-2 col-md-7 col-md-offset-2">
                        <div class="panel panel-default">
                            <div class="panel-heading">Repay Debt</div>
                            <div class="panel-body">
                                <form class="form-horizontal" data-toggle="validator" id="form_return_request_product" method="post">
                                    <div class="form-group">
                                        <label for="inputEmail3" class="col-sm-3 control-label">Select Supplier Name</label>
                                        <div class="col-sm-9">
                                           <select class="form-control" id="select_request_item" name="select_request_item">
                                            <option value="" disabled="disabled" selected="selected">Select Supplier Name</option>

                                                <?php  $get_suppliers_query = mysqli_query($connection,"SELECT * FROM `tblsuppliers`");
                                                    while ($supplier_names = mysqli_fetch_array($get_suppliers_query)) {  ?>

                                                <option value="<?php  echo $supplier_names['supplierid']  ?>"><?php   echo $supplier_names['orgname']; ?></option>                
                                                 <?php   }
                                                    ?>
                                               
                                            </select>
                                        </div>
                                        <div class="help-block with-errors" style="text-align: center"></div>
                                    </div>
                                
                                    

                            
                                    <div class="form-group" style="margin-bottom:0">
                                        <div class="col-sm-offset-2 col-sm-10 text-center">
                                            <button type="submit" class="btn btn-primary" name="btn_generate_list">Get Products</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>


                <?php

                if(isset($_POST['btn_generate_list'])){ 

                    $get_selected_item = $_POST["select_request_item"];   

                 ?>

                 <div class="row cm-fix-height">
                    <div class="col-lg-7 col-lg-offset-2 col-md-7 col-md-offset-2">
                        <div class="panel panel-default">
                            <div class="panel-body">
                                <form class="form-horizontal" data-toggle="validator" id="form_make_payment" method="post">

                                    <input type="hidden" name="" id="hidden_supplier" value="<?php  echo $get_selected_item; ?>">

                                    <input type="hidden" name="" id="hidden_payed" value="">

                                    <div class="form-group">
                                        <label for="inputEmail3" class="col-sm-3 control-label">Select Product</label>
                                        <div class="col-sm-9">
                                           <select class="form-control" id="selected_product_name_due" name="selected_product_name_due">
                                            <option value="" selected="selected" disabled="disabled">Select Product</option>

                                                <?php
                                                $get_products_query = mysqli_query($connection,"SELECT * FROM suppliers_table JOIN stock_product_table ON suppliers_table.supplier_product = stock_product_table.stock_product_id WHERE supplier_name = '$get_selected_item'");

                                                    while ($product_name = mysqli_fetch_array($get_products_query)) {  ?>

                                                <option value="<?php  echo $product_name['supplier_product']  ?>"><?php   echo $product_name['stock_product_name']; ?></option>                
                                                 <?php   }
                                                    ?>
                                               
                                            </select>
                                        </div>
                                        <div class="help-block with-errors" style="text-align: center"></div>
                                    </div>

                                     <div class="form-group">
                                        <label for="inputEmail3" class="col-sm-3 control-label">Supplier Selected</label>
                                        <div class="col-sm-9">
                                            <input type="text" data-error="Enter Quantity To Transfer" class="form-control" id="txt_item_selected" placeholder="Transfer Quantity" required="required" disabled="disabled" name="txt_item_selected" value="<?php 
                                            $run_query_2 = mysqli_query($connection,"SELECT * FROM suppliers_table JOIN tblsuppliers ON suppliers_table.supplier_name = tblsuppliers.supplierid WHERE supplier_name = '$get_selected_item'");

                                             $get_supplier_name = mysqli_fetch_array($run_query_2);
                                             $supplier_name = $get_supplier_name["orgname"];
                                            echo $supplier_name;  ?>">
                                        </div>
                                        <div class="help-block with-errors" style="text-align: center"></div>
                                    </div>


                                    <div class="form-group">
                                        <label for="inputEmail3" class="col-sm-3 control-label">Amount Due</label>
                                        <div class="col-sm-9">
                                            <input type="decimal" data-error="Enter Amount Due" class="form-control" id="txt_amount_due" placeholder="Amount Due" required="required"  name="txt_amount_due" disabled="disabled">
                                        </div>
                                        <div class="help-block with-errors" style="text-align: center"></div>
                                    </div>

                                    <div class="form-group">
                                        <label for="inputEmail3" class="col-sm-3 control-label">Amount Paying</label>
                                        <div class="col-sm-9">
                                            <input type="text" data-error="Enter Amount Paying" class="form-control" id="txt_amount_paying" placeholder="Amount Paying" required="required" name="txt_amount_paying">
                                        </div>
                                        <div class="help-block with-errors" style="text-align: center"></div>
                                    </div>

                                                 
                            
                                    <div class="form-group" style="margin-bottom:0">
                                        <div class="col-sm-offset-2 col-sm-10 text-center">
                                            <button type="submit" class="btn btn-primary">Purchase Item</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                </div>


              <?php  }else{} ?>
               
                
            </div>

<?php	}  ?>
  

?>

                    